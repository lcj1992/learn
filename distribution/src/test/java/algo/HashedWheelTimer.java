...
public class HashedWheelTimer implements Timer {
    ...
    private final Queue<HashedWheelTimerFuture> waitingTasks = new LinkedBlockingDeque<>();
    private final Queue<HashedWheelTimerFuture> canceledTasks = new LinkedBlockingDeque<>();
    ...
    public HashedWheelTimer(long tickDuration, int ticksPerWheel, int processThreadNum) {
        ...
        if (processThreadNum <= 0) {
            ...
            // 这里需要调整一下队列大小
            BlockingQueue<Runnable> queue = new LinkedBlockingQueue<>(8192);
            int core = Math.max(Runtime.getRuntime().availableProcessors(), processThreadNum);
            // 基本都是 io 密集型任务
            taskProcessPool = new ThreadPoolExecutor(core, 2 * core, 60, TimeUnit.SECONDS, queue, threadFactory, new ThreadPoolExecutor.CallerRunsPolicy() {
            });
        }
